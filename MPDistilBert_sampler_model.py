...
class SamplingLayer(nn.Module):
    def __init__(self, topk_entities, topk_predicates):
        super(SamplingLayer, self).__init__()
        self.top_e = topk_entities
        self.top_p = topk_predicates
        
    def forward(self, e_scores, entity_ids, p_scores, all_predicate_ids, answer=None):
        '''
        Inputs:
...
#             kg.configure_hops(1, sampled_predicates, 'predef-wikidata2018-09-all', True, False)
            # call function to get a subgraph
            s_entity_ids, s_predicate_ids, adjacencies = compute_hops(sampled_entities, sampled_predicates, 5000, 0)
#             kg.remove()
#             del kg
...
#         print(p_scores)
        ...
        subgraph = torch.sparse.FloatTensor(indices=indices, values=p_scores,
                                            size=(num_entities, num_entities*num_relations)).to('cuda')
...
#         self.pre_classifier = nn.Linear(config.hidden_size, bottleneck_dim)
        ...
        self.subgraph_sampling = SamplingLayer(topk_entities, topk_predicates)
